...
describe('gate graph validation', () => {
  ...
  it('keeps the hygiene aggregate aligned with the package script checks', () => {
    ...
    expect(ids).toEqual([
      ...
      'optional-dependency-imports', 'client-packages', 'client-ui-i18n', 'client-css-tokens', 'no-bare-dispatcher', 'cordis-config',
  ...
  it.each(['ci-primary', 'ci-static', 'check-all', 'hygiene'] as const)(
    'keeps undeclared --dsw token enforcement in %s',
    (mode) => {
      const ids = withPnpmEntrypoint(() => gatesForMode(mode).map(subject => subject.id))

      expect(ids).toContain('client-css-tokens')
    },
  )
