...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  'packages/ui/tui': { kind: 'none', reason: 'Node terminal presentation layer; registers no prompt, tool schema, or provider-request content.' },
  ...
  'packages/client/connection-vscode': { kind: 'none', reason: 'The VS Code carrier transports existing messages but contributes no model context.' },
  ...
  'packages/host/client-modules-web': { kind: 'none', reason: 'The Web adapter serves Client Plugin artifacts and boot metadata but registers nothing model-facing.' },
  ...
  'packages/bundle/client-app': { kind: 'indirect', reason: 'The bundle is a patch-list carrier; each inserted row\'s package owns its model-facing behavior.' },
