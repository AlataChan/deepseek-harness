...
The throwaway consumer declares every supplied tarball as both a direct `file:` dependency and an npm override that references that direct dependency. The direct dependency selects the artifact under test; the override also redirects transitive semver edges to it. Without both declarations, npm may query the registry for a transitive dsh range and reject a version that cannot exist there until this verification passes and publication begins. An override replaces npm's range selection, so `check-workspace-constraints`, release verification, and pack substitution own the published internal ranges; this probe owns payload completeness and execution from installed bytes.

The verification also supplies a packed Landlock entry, which `dsh-sandbox-local` declares as a plain dependency, and leaves optional dependencies enabled. npm uses each optional platform package's `os` and `cpu` fields to install only the compatible payload; the matching Landlock platform package may resolve from the registry because a DSH release does not rebuild it, while platform payloads from external dependencies such as Koffi must remain enabled for the rehearsal to match a real consumer install. The verification therefore reads a directory by its contents rather than a pack order, because a directory can hold tarballs packed only to satisfy a cross-sequence dependency.
...
**Running a local registry for packed-install verification.** A local registry would exercise npm's registry-backed selection for every internal range. It also adds a stateful service and a second publication process to a job whose installed-artifact probe owns payload completeness and startup. Workspace constraints, release verification, and pack substitution already own the internal range spelling, so the probe keeps every internal edge on the supplied tarballs instead.

**Direct `file:` dependencies without npm overrides.** A root tarball does not stop npm from resolving a transitive semver edge through the registry. That fails before first publication because the target version is correctly absent there. Referencing each direct dependency from `overrides` keeps resolution on the verified tarballs without running a registry.
