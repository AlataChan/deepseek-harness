...
This reference defines the profile, web-alias, plugin-management, config-dump, and VS Code companion modes. Public CLI argv is parsed once through [`src/args.ts`](../src/args.ts), and [`src/bin.ts`](../src/bin.ts) dynamically imports only the selected runner; the extension forks the separately published companion module directly.
...
| `vscode` | `--workspace-root <absolute-path>`; internal companion argument |
...
The `@deepseek-ai/dsh` manifest publishes the VS Code companion at `dsh.companions.vscode`. The extension resolves that module from the installed runtime and forks it with Node directly, so Windows never executes a `.cmd` or PowerShell PATH shim. The child requires Node's built-in IPC channel and carries all bounded protocol records through it; it does not use numbered file descriptors. `--workspace-root` is required, absolute, and the only accepted startup argument. The companion changes to that root before loading project environment values and boots only the shipped `vscode` profile.

Before profile boot, the companion exclusively creates `$DSH_HOME/.locks/vscode-companion.lock`. The record includes its format version, PID, random instance id, start time, and resolved home. A live owner, corrupt record, unreadable record, or indeterminate PID probe fails closed with carrier error `home-busy`; record age never breaks ownership. A definitely dead owner is moved to a uniquely named stale record before one exclusive retry. Clean shutdown removes the active path only while its owner token still matches. Consequently, two VS Code windows may target the same workspace only when they use different Harness homes; two companions never write one home concurrently.
