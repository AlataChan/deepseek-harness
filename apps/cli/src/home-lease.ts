/** Exclusive lifecycle lease for one companion's resolved DSH_HOME. */
...
const HOME_LEASE_VERSION = 1

/** Lease location relative to the resolved Harness home. */
export const HOME_LEASE_RELATIVE_PATH = join('.locks', 'companion.lock')

/** Surfaces that may hold the home-wide companion lease. */
type HomeLeaseSurface = 'vscode' | 'desktop'

/** Durable owner facts used for liveness checks and token-matched release. */
export interface HomeLeaseRecord {
  ...
  home: string
  /** Surface that acquired this lease. */
  surface: HomeLeaseSurface
}
...
export interface HomeLeaseOptions {
  /** Surface requesting exclusive home ownership. */
  surface: HomeLeaseSurface
  /** Owning process id. */
  pid?: number
...
export interface HomeLease {
  ...
  readonly owner: Readonly<HomeLeaseRecord>
...
function isSurface(value: unknown): value is HomeLeaseSurface {
  return value === 'vscode' || value === 'desktop'
}

function parseRecord(value: unknown, expectedHome: string): HomeLeaseRecord | undefined {
  if (typeof value !== 'object' || value === null) return undefined
  const record = value as Partial<HomeLeaseRecord>
  if (record.version !== HOME_LEASE_VERSION
    ...
    || record.home !== expectedHome
    || !isSurface(record.surface)) return undefined
  return record as HomeLeaseRecord
}

function readRecord(path: string, home: string): HomeLeaseRecord | undefined {
  try {
...
function sameOwner(left: HomeLeaseRecord | undefined, right: HomeLeaseRecord): boolean {
...
function writeExclusive(path: string, owner: HomeLeaseRecord): void {
...
function busy(home: string, owner?: HomeLeaseRecord): HomeBusyError {
  const detail = owner === undefined
    ? 'owner record is corrupt or unreadable'
    : `${owner.surface} companion process ${String(owner.pid)} owns it`
  return new HomeBusyError(`home-busy: companion cannot use ${home}: ${detail}`)
}
 ...
 * @param options - required surface plus injectable owner and liveness facts.
 * @returns the acquired token-matched lease handle.
 */
export function acquireHomeLease(
  home: string,
  options: HomeLeaseOptions,
): HomeLease {
  const resolvedHome = resolve(home)
  const path = join(resolvedHome, HOME_LEASE_RELATIVE_PATH)
  const recoveryPath = `${path}.recovery`
  mkdirSync(dirname(path), { recursive: true, mode: 0o700 })
  const owner: HomeLeaseRecord = Object.freeze({
    version: HOME_LEASE_VERSION,
