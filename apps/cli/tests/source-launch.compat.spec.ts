 ...
 * non-TTY TUI diagnostic. The Node compatibility matrix runs this
...
describe('dsh SOURCE launcher (node --import tsx/esm)', () => {
  ...
  it('boots the source entry and rejects a non-interactive default TUI', async () => {
    const dshHome = await mkdtemp(join(tmpdir(), 'dsh-source-launch-'))
    try {
      ...
      expect(result.stderr).toContain('requires interactive stdin and stdout; use dsh exec')
