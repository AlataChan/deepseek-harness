...
use dsh_desktop::{
    forward_carrier_events, CacheBundleArgs, DesktopShell, RuntimeConfig,
};
...
fn reply_err(out: &Mutex<std::io::Stdout>, id: u64, error: impl ToString) -> Result<(), String> {
    write_line(out, &json!({ "id": id, "ok": false, "error": error.to_string() }))
}
...
fn dispatch(
    ...
    match request.cmd.as_str() {
        "runtime_configure" => {
            let args: ConfigureArgs = serde_json::from_value(request.args).map_err(|error| error.to_string())?;
            match guard.configure(&RuntimeConfig {
                node_path: args.node_path,
                runtime_path: args.runtime_path,
                workspace_root: args.workspace_root,
            }) {
                Ok(config) => reply_ok(out, request.id, serde_json::to_value(config).map_err(|error| error.to_string())?),
        ...
        "runtime_resolve" => match guard.resolve() {
            Ok(resolved) => reply_ok(out, request.id, serde_json::to_value(resolved).map_err(|error| error.to_string())?),
            Err(error) => reply_err(out, request.id, error),
        },
        ...
        "carrier_send" => {
            let args: SendArgs = serde_json::from_value(request.args).map_err(|error| error.to_string())?;
        ...
        "cache_bundle" => {
            let args: CacheBundleArgs = serde_json::from_value(request.args).map_err(|error| error.to_string())?;
            match guard.cache_bundle_from_args(&args) {
                Ok(result) => reply_ok(out, request.id, serde_json::to_value(result).map_err(|error| error.to_string())?),
                Err(error) => reply_err(out, request.id, error),
            }
        }
...
fn main() {
    let home = env_dir(
        ...
        std::env::current_dir().expect("cwd").join(".dsh-desktop-harness"),
    );
    ...
    let shell = Arc::new(Mutex::new(DesktopShell::new(config_dir, cache_root, cli_path, path_value, dsh_desktop::runtime::BundledResources::default())));
