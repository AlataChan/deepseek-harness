...
Client graph configs reach the Webview as JSON. The VS Code build replaces the vendored Loader's JavaScript-expression evaluator with a refusal so its strict content security policy never requires dynamic code execution. Because a Webview has no Node `process` global, the build also substitutes a compile-time `process.env` object containing only the production marker. VSIX verification scans every Webview JavaScript asset and rejects Function constructors, direct `eval`, or remaining `process` access.
