...
`host/` 组提供 Web GUI 的普通 HTTP 服务器、服务已构建 Web 壳的 SPA dist 服务器、带原生／浏览／自适应组合包的工作区目录选择 seam、单层工作区文件列表 seam、问数列表／导入／绑定 seam、商务数据源／导入／绑定／读取／分析 seam、问知识目录／入库／挂接／检索 seam，以及只读的插件清单投影。这十一个包都是产品包；浏览器传输位于 [`client/`](../client/README.zh.md)，组合应用是 [`apps/cli`](../../apps/cli/README.zh.md)，它启动 [`dsh-base` 组合包](../bundle/base/cordis.patch.yml) 来提供 `apps/web/` 下的 Web 应用。选择器后端可在共享 seam 后互相替换。
...
十一个包分别承担 Host 角色；各包的 README 拥有自己的约定与配置。
...
| [`workspace-entries/`](workspace-entries/README.zh.md) | 单层项目文件列表 seam | `ctx.workspaceEntries` |
| [`ask-data/`](ask-data/README.zh.md) | 问数列表、导入与 Session 绑定 seam | `ctx.askData` |
| [`commerce/`](commerce/README.zh.md) | 商务数据源、导入、Session 绑定、读取、分析与导出渲染 seam | `ctx.commerce` |
| [`ask-knowledge/`](ask-knowledge/README.zh.md) | 问知识目录、入库、挂接与检索 seam | `ctx.askKnowledge` |
