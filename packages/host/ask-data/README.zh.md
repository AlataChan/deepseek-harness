...
Client 聚合通过 `@deepseek-ai/dsh-host-ask-data/client` 取得 `askDataBinding` 投影合并，也取得 `AskDataErrorCode` —— 线上以 `details.code` 携带的封闭失败 code 联合类型，每个面向用户的界面都要靠它挑文案；该出口转述 `./types`，不加载 Host `AskData` 服务。
