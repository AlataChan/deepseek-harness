...
每个 endpoint 都声明自己的激活策略。列表、搜索、附件、历史页、日志跟随、skill 发现、工作区路径打开和 `listEntries` 可以在不激活 Agent 的情况下检查 persistence；`canOpenWorkspacePath()` 无需指定 Session 即可报告原生打开能力。`listEntries` 通过 `ctx.workspaceEntries` 把一层目录围在指定 Session 的 `cwd` 内，能力缺失时失败为 `session/entries-unavailable`。queue 变更与取消要求 live 状态；模型、重命名、prompt 和文件引用操作可以解析或恢复普通 Session。只有 create 与 fork 会直接创建新 Agent。skill 目录则优先使用已有 live Agent，否则使用所记录 preset 的常驻 scope，因此列表查询绝不会启动 Agent。

四个电商 Remote 在 `ctx.commerce` 之上沿用同一套模式：`listCommerceSources`、`importCommerceSpreadsheet` 与 `importCommerceSample` 不打开任何 Session；`commitCommerce` 把一个数据源绑定到空白会话或新建会话，并让它运行 `commerce` preset，绑定失败时恢复原 preset 并销毁自己创建的会话。没有电商能力的 Host 返回 `session/commerce-unavailable`，没有 `commerce` preset 的 Host 返回 `session/commerce-preset-unavailable`。
