...
Each endpoint states its activation policy. List, search, attachment, history pages, log following, skill discovery, workspace-path opening, and `listEntries` can inspect persistence without activating an Agent; `canOpenWorkspacePath()` reports native-opening availability without addressing a Session. `listEntries` fences one directory to the named Session's `cwd` through `ctx.workspaceEntries` and fails as `session/entries-unavailable` when that capability is absent. Queue mutation and cancellation require live state; model, rename, prompt, and file-reference operations may resolve or resume an ordinary Session. Create and fork are the only operations that create a new Agent directly. The skill catalog instead uses a live Agent when present or the recorded preset's standing scope when cold, so listing never starts an Agent.

Four commerce remotes follow the same pattern over `ctx.commerce`: `listCommerceSources`, `importCommerceSpreadsheet`, and `importCommerceSample` open no Session, while `commitCommerce` binds one source to a blank or newly created Session on the `commerce` preset and compensates a failed bind by restoring the previous preset and disposing a Session it created. A Host without the commerce capability answers `session/commerce-unavailable`, and one without the `commerce` preset answers `session/commerce-preset-unavailable`.
