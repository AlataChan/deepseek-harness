/**
 * First grapheme and a stable hue for a data-source avatar.
 * @module @deepseek-ai/dsh-experimental-desktop-ask-data/client/source-initial
 */

const HUE_COUNT = 3

/**
 * First grapheme of a source name for the identity avatar.
 * @param displayName - listed source name.
 * @returns one grapheme, or `?` when the name is empty.
 */
export function sourceInitial(displayName: string): string {
 ...
 * @returns 0, 1, or 2.
 */
export function sourceHueIndex(displayName: string): 0 | 1 | 2 {
  let hash = 0
