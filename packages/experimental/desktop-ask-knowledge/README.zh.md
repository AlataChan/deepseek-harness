...
活库在 Tauri `app_data_dir`（`OCTOPUS_APP_DATA` / `knowledgeHome`）。`catalog.json` 是唯一户口本。工作文件夹可以放可选 symlink，那不是索引。propose 从 `ctx.credentials.resolve` 取 `DEEPSEEK_API_KEY`，只注入该次 sidecar 子进程。sidecar 可执行文件按 `sidecarRuntimePath` 再 `OCTOPUS_SIDECAR_HOME` 解析。检索使用官方 SD 的 `terms[]`。完整工具结果有界。上传走 `beginIngest` / `appendIngestChunk` / `finishIngest`。`finishIngest` 会跑转换、LLM propose 和 apply；桌面 carrier 只给这一条方法 180 秒。仅本会话抽取走 `beginExtract` / `appendExtractChunk` / `finishExtract`；`finishExtract` 只跑 sidecar `convert-file`，等 90 秒，不写户口本。选库面板点库名挂上已有库，点「添加文档」往同一个库再放一份材料，点「删除」从名单去掉。选择器和设置名单行带字母头像、「文档库」徽章，以及已入库的 `raw/*.md` 篇数。未绑定时 hero chip 用文件夹图标，绑定时改成字母头像。批次状态在文案旁用共用图标集。hero chip 再点一次收起 Modal 选择器；Escape 和 Modal 关闭控件都会关掉它。若当前是已绑表的数据模式会话，点库名会新开一个标准会话再挂上；数据模式会禁掉检索工具。propose 会在 apply 前补上 `create_page` 缺的 `role` / `layer` / wiki `summary`，但不再把 tags 写进 aliases：同一份文档有两个页面共享某个 tag 时，那一个别名会指向多个页面，post-apply lint 会因此整份打回。apply 也会补这些字段。别名冲突检测跳过 source 页 —— 入库的源文件和由它整理出的 wiki 页本来就共用文档名 —— 而 append / alias 操作会沿用 vault 对同一路径的既有大小写拼法。被 lint 打回的提案会把规则和页面写进拒件、并报给选择器，不再是一句话概括所有原因。`deferred` 的提案其实什么都没写（词条停在一个没有任何界面会看的口袋队列里），所以选择器报「没写进去」而不是「已入库」。出于同样理由，`builtins.yaml` 删掉了 `confidence.tier_gate_defer` 规则，中等置信度的提案会直接入库而不是停进那个队列；低于 0.4 的拒绝门保留。挂库时会修复磁盘上已有的拒件并重跑，其中包括摘掉退役 fill 从页面自身 tags 复制出来的别名 —— 所以这些修复之前被拒的文档不必重新上传就能变成可检索。挂库时会把只因 page-meta 被拒的提案再 apply 一次，让 wiki digest 落地。检索对查询、标题和正文做 NFKC 折叠；`ask_knowledge_retrieve` 把命中页正文渲染给模型。lookup 在词条名和输入不完全相同时，按标题、别名、标签或 retrieve 命中解析。新建进入上传面板；选文件控件是盖在按钮上的透明 file input，这样 Tauri WebView 才能打开本机选择器。该 input 不设 HTML accept，并在元素上听 change，以便 WebKit 交出 File。它可以一次收多份文件，按顺序逐份入库到同一个库 —— 因为 Host 对每个库存取入库是串行的，而一份文件要吃一个 180 秒的 carrier 预算；每份文件各有自己的「排队中 / 正在处理 / 已入库 / 失败原因」一行，其中一份失败不会中断其余的。队列在浏览器里，所以关掉窗口会丢掉还没轮到的那几份。取消选文件不会写户口本；选中文件或点「先空着，直接提问」才建库。加号菜单打开选库列表，这样文档会进选中的库，而不是一份文件建一个新库。默认名是「未命名知识库」，不是按钮文案；第一次入库成功后按文件名去扩展名改名，往已有命名库添加不改名。`finishIngest` 失败时显示 `error`，并把 sidecar 里已知的英文映射成中文。propose 会截短过长原文，并要求短的 `create_page` 正文。composer 一直可见。加号菜单的「添加到知识库」通过 `conversation.input.attachKnowledge` 打开同一选择器，「看这份文档（仅本会话）」通过 `conversation.input.attachSessionDocument` 把抽出正文放在 composer 芯片上，发送时写入正文。仅本会话的 Word 由 Host 解 `word/document.xml`；PDF 和 HTML 仍走 sidecar `convert-file`。知识库入库同样收 `.docx`：Host 先解成 markdown，再交给 sidecar `ingest-file`。Client 工厂不引进 `fflate`。
...
- **不接受老 .doc** — Word 须是 `.docx`。Host 只读 `word/document.xml`；其它 zip 段里的页眉、页脚、批注不会抽出
