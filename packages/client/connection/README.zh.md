...
- **`/api` 桥把每个请求体整体缓冲在内存里**：`maxRequestBodyBytes`（默认 160 MiB，按默认 100 MiB 图片总量上限经 base64 膨胀加信封余量得出）因此同时是单请求的驻留内存上界；VS Code 载体也将此容量作为默认逻辑 RPC 上限，要降低它而不缩小图片限额，需要流式路径或由分块层持有附件的路径。
