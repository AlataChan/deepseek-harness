...
Node 侧只要求 Loader。它扫描已启用配置项以发现与 Web 兼容的 `dsh.client` 包，解析每个 `exports["./client"]`，把构建后的 bundle 哈希和包专属 `dsh.client.external` 请求写入 `ClientBootGraph`，把动态提供方排在消费者之前，还通过只读的 `bundleRecords()` 快照把每个图 entry 与其绝对产物路径配对。源码启动会把宿主侧导入映射到 TypeScript 源码，但仍消费这一构建后的客户端导出；缺失文件共享一条构建说明，随后以包／路径列表列出各项，而无关的文件系统错误仍是独立故障。表层适配器决定如何把这些记录交给客户端：[dsh-host-client-modules-web](../../host/client-modules-web/README.md) 提供 `/plugins` 并注入 bootstrap 脚本与 `window.__DSH_BOOT__`，非 HTTP 客户端则直接消费同一份图和路径。
