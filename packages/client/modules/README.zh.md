...
每个界面适配器都会在 parser preload 运行前安装 `window.__ModuleLoader__`。公开的 `bootstrap-ids` 子路径提供共享 preload 名录与 queue facade 构造器，使 Web、VS Code 和装配测试选择相同的 bootstrap 行。facade 的 queue 模式 `load()` 保存提前到达的 registration；`create()` 使用拒绝 external 的 bootstrap require 物化本包 factory，并调用其 `createClientModuleSystem` 导出。构造过程把同一组导出缓存为 modules row，把同一个 facade 切换到 live registration，再排空余下 queue。Bundle 通过模块闭包保留生成的系统，因此随后 Cordis `apply()` 能把同一实例提供为 `ctx.modules`，无需另一个页面全局变量。
...
Node 侧只要求 Loader。它扫描已启用配置项以发现与 Web 兼容的 `dsh.client` 包，解析每个 `exports["./client"]`，把构建后的 bundle 哈希和包专属 `dsh.client.external` 请求写入 `ClientBootGraph`，把动态提供方排在消费者之前，还通过只读的 `bundleRecords()` 快照把每个图 entry 与其绝对产物路径配对。无关联行之间以 Loader entry 顺序打破平局，bundle 记录也采用这一顺序，因此异步 fiber 激活不会改变图修订号或表层握手。源码启动会把宿主侧导入映射到 TypeScript 源码，但仍消费这一构建后的客户端导出；缺失文件共享一条构建说明，随后以包／路径列表列出各项，而无关的文件系统错误仍是独立故障。表层适配器决定如何把这些记录交给客户端：[dsh-host-client-modules-web](../../host/client-modules-web/README.zh.md) 提供 `/plugins` 并注入 bootstrap 脚本与 `window.__DSH_BOOT__`，非 HTTP 客户端则直接消费同一份图和路径。
