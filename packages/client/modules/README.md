...
The Node half requires only Loader. It scans enabled entries for Web-compatible `dsh.client` packages, resolves each `exports["./client"]`, hashes the built bundle into `ClientBootGraph`, carries package-specific `dsh.client.external` requests, orders dynamic providers before consumers, and exposes a read-only `bundleRecords()` snapshot pairing every graph entry with its absolute artifact path. Source launch maps host imports to TypeScript source but still consumes this built client export; missing files share one build instruction followed by a package/path list, while unrelated filesystem errors remain separate failures. Surface adapters decide how those records reach a client: [`dsh-host-client-modules-web`](../../host/client-modules-web/README.md) serves `/plugins` and injects the bootstrap scripts plus `window.__DSH_BOOT__`, while non-HTTP clients consume the same graph and paths directly.
