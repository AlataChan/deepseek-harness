...
describe('ui-sidebar apply', () => {
  ...
  it('registers the shell and declares its child seats', async () => {
    ...
    expect(b.slots.spec('sidebar.files')).toEqual({ kind: 'single', scope: 'root' })
    ...
    expect(injected.hooks.filesOccupied.getSnapshot()).toBe(false)
    const notified = vi.fn()
    const unsubscribe = injected.hooks.filesOccupied.subscribe(notified)
    const dispose = b.slots.register({ name: 'sidebar.files' } as never, () => null)
    expect(injected.hooks.filesOccupied.getSnapshot()).toBe(true)
  ...
  it('removes the entry and child declaration on teardown', async () => {
    ...
    expect(b.slots.spec('sidebar.files')).toBeUndefined()
