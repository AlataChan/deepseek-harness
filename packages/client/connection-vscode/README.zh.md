...
与浏览器兼容的 `protocol` 和 `codec` 入口保持独立于 Node。Host 根插件消费进程 IPC channel、校验启动握手、公布经过验证的 Client Plugin 产物、路由既有 ApiProxy envelope，并通过其 Cordis fiber 排空 stream pump。Webview port 与客户端 controller 由下一个运行时任务添加。
