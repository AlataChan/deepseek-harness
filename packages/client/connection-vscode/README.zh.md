...
与浏览器兼容的 `protocol` 和 `codec` 入口保持独立于 Node。Host 根插件消费进程 IPC channel，依据配置的 `workspaceRoot` 校验启动握手（未配置的嵌入场景使用 `process.cwd()`），公布经过验证的 Client Plugin 产物、发布 Host Connection 服务，并通过其共享 `/api` handler 路由 unary 请求。该 handler 先运行已认领的 Typert Remote endpoint，再回退到 ApiProxy；方法路径分段保持独立，使 `namespace/method` 请求能够到达对应认领项。插件通过其 Cordis fiber 排空 stream pump。`client` 入口消费由 shell 私下提供的 record port 与 IDE event port、扩展既有 `AbstractApiClient`，并公布标准 Client `ctx.connection` handle。它关联有界 unary 调用与 receipt，只向上游打开 `mux` 和 Host 生命周期 frame，所有 stream 数据只从下行接收。runtime 代际进入 stopping、restarting 或 failed 状态时，它会拒绝待处理的 unary 调用并结束 stream，但不会永久关闭 API 客户端；共享 `ConnectionController` 负责针对下一代 ready runtime 重新打开两条 stream。
