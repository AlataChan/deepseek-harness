...
`Modal` 与导出的 `useModalFocus` hook 把键盘归属交给最后挂载的 `aria-modal` 弹窗。它们把焦点移动到调用方指定目标、首个可聚焦后代或弹窗本身；在弹窗两端回绕 Tab 与 Shift-Tab；只把 Escape 路由给最上层弹窗；并在清理时把焦点恢复到仍挂载的触发控件。自定义弹窗使用该 hook 时，须把返回的 ref 绑定到同时携带 `role="dialog"` 与 `aria-modal="true"` 的元素。
