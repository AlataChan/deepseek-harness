...
2. **Three registration surfaces, all required** (missing any one fails at a different, later point): the `tsconfig.client.json` aggregate `references` entry; a `dsh.client` row in the owning composition bundle (`packages/bundle/client-app/cordis.patch.yml` for shared interactive behavior, otherwise the surface bundle); and a dependency in that same bundle's `package.json` (profile boots resolve bare row names through the healed `$DSH_HOME/profiles/node_modules` fallback, which mirrors the app's and each bundle's declared dependencies — a row whose package no manifest declares fails to import). Keep surface-private rows in their surface bundle: for example, `connection-vscode` belongs to `vscode-app`, not `client-app`. `pnpm-workspace.yaml` already globs `packages/*/*`.
