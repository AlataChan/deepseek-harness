...
The settings panel keeps its two-column navigation and content layout above 640 CSS pixels. At 640 pixels and below, including the default VS Code Activity Bar width, the navigation becomes a compact horizontally scrollable row above a full-width content column. The compact sidebar trigger keeps its translated accessible name as visually hidden text when its visible label is removed.
