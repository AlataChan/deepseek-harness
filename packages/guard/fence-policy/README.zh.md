...
纯文本大小处理遵循 `dsh-spill-policy` 的尽力而为契约：它以前置方式注册的执行后监听器会先委托，收到完整围栏结果，并在存储成功时溢出或生成预览。若会话所有者或溢出后端不存在，或者 `saveText` 失败，完整围栏结果会保留在行内，而对抗性转义可能让文本扩大到 8 倍。结果只要含有非文本块，溢出策略就不会限制它，因此本包用 `maxMixedTextBytes` 对围栏文本聚合大小实施硬上限，同时将非文本块保留在原位。
...
`sanitizeUntrusted` 在不执行 Unicode 规范化的前提下进行一次线性扫描。它移除指定不可见码位，把不允许的 C0/C1 控制符替换为空格，仅在结构位置转义分隔符起始字符和字符引用的 `&`，并转义行首角色标记的冒号。普通比较表达式、URL 查询字符串、shell 重定向和自然语言标点保持不变；名称恰好为 `lt` 或 `amp` 的查询键（如 `?x=1&lt=3`）会被转义，因为它与无分号旧式字符引用具有相同语法。
...
系统区段字节稳定且只添加一次。每个选定文本块增加两行包裹文本；结构转义使结果文本最多扩展到 8 倍。只有尽力而为的存储路径成功时，溢出策略才会限制纯文本结果；对于混合结果，`maxMixedTextBytes` 始终限制围栏文本的聚合大小，其中包括包裹文本和截断后缀。
...
- **纯文本溢出失败**——纯文本围栏没有独立字节上限。若会话所有者或后端不存在，或存储拒绝导致溢出策略无法保存超大结果，完整围栏文本会保留在行内，大小可能达到原始文本的 8 倍；混合结果仍由 `maxMixedTextBytes` 实施硬上限。
