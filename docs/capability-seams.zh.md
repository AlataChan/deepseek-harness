...
flowchart LR
  pkg_tui_app["tui-app"]
  svc_tuiStartup["ctx.tuiStartup<br/>Terminal startup intent"]
  pkg_tui["tui"]
...
| --- | --- | --- | --- | --- | --- | --- |
| `ctx.tuiStartup` | `bundle` | [`tui-app`](../packages/bundle/tui-app) | - | [`tui`](../packages/ui/tui) | - | TUI profile 只解析一次应用参数，并将选定的新建或恢复启动意图提供给终端 runtime。 |
